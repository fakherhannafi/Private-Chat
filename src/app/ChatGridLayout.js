...
class ChatGridLayout extends React.Component {
  constructor(props) {
    ...
    this.state = {
      username: "",
      icon: "",
      friendname: "mon Guide",
      friendicon: "./assets/bot.png",
      subheader:
        "Sélectionnez, à gauche, la personne avec laquelle vous voulez discuter",
      messageList: [],
      friendList: [],
  ...
  componentDidMount() {
    ...
    if (retrievedObject == null) {
      window.alert("La connexion est invalide, veuillez vous reconnecter!");
  ...
  addMsgCallback = item => {
    ...
    for (var i = 0; i < tmp.length; i++) {
      if (tmp[i].friendname === item.authorName) {
        ...
        tmp[i].lastMsg = "Moi: " + item.msg + " (" + item.time + ")";
        // tmp[i].notifNum += 1;
        this.setState({ friendList: tmp }, () => {
          // console.log(this.state.friendList);
        });
    ...
    this.setState(
      {
        ...
        subheader: "Dans la salle de discussion en ligne"
      },
      () => {
        ...
        <Grid container className={classes.root} spacing={24}>
          ...
          <Grid item xs={12} sm={9}>
            <ChatRoomLayout
              ...
              username={this.state.username}
              friendname={this.state.friendname}
              friendicon={this.state.friendicon}
              subheader={this.state.subheader}
              icon={this.state.icon}
              messageList={this.state.messageList}
              pickFriendBoolean={this.state.pickFriendBoolean}
              addMsgCallback={this.addMsgCallback}
              addNotifCallback={this.addNotifCallback}
            />
          </Grid>
        </Grid>
      </div>
